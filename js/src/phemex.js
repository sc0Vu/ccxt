...
export default class phemex extends Exchange {
    ...
    async fetchMarkets(params = {}) {
         ...
         */
        let oldReq = await this.publicGetExchangePublicCfgV2Products(params);
        let oldReqData = oldReq['data'];
        let newReq = await this.publicGetPublicProducts(params);
        let newReqData = newReq['data'];
        let oldCurrencies = oldReqData['currencies'];
        let oldCurrenciesPair = []
        let newCurrencies = newReqData['currencies'];
        let newCurrenciesPair = []
        for (let i=0;i<oldCurrencies.length;i++){
            oldCurrenciesPair.push(oldCurrencies[i]['currency'])
        }
        for (let i=0;i<newCurrencies.length;i++){
            let dd = newCurrencies[i]['currency']
            let index = oldCurrenciesPair.indexOf(dd)
            if (index>-1){
                oldCurrenciesPair.splice(index, 1)
            } else {
                newCurrenciesPair.push(dd)
            }
        }
        let oldProducts = oldReqData['products'];
        let oldProductsPair = []
        let newProducts1 = newReqData['products'];
        let newProducts2 = newReqData['perpProductsV2'];
        let newProducts = newProducts1.concat(newProducts2)
        let newProductsPair = []
        for (let i=0;i<oldProducts.length;i++){
            oldProductsPair.push(oldProducts[i]['symbol'])
        }
        for (let i=0;i<newProducts.length;i++){
            let dd = newProducts[i]['symbol']
            let index = oldProductsPair.indexOf(dd)
            if (index>-1){
                oldProductsPair.splice(index, 1)
            } else {
                newProductsPair.push(dd)
            }
        }
        let oldRiskLimits = oldReqData['riskLimits'];
        let newRiskLimits = newReqData['riskLimits'];
        let oldRiskLimitsV2 = oldReqData['riskLimitsV2'];
        let newRiskLimitsV2 = newReqData['riskLimitsV2'];
        let oldLeverages = oldReqData['leverages'];
        let newLeverages = newReqData['leveragesV2'];
        return {
            "currencies": {
                "old": oldCurrencies.length,
                "new": newCurrencies.length,
                oldCurrenciesPair,
                newCurrenciesPair
            },
            "products": {
                "old": oldProducts.length,
                "new": {
                    "riskLimits": newProducts1.length,
                    "perpProductsV2": newProducts2.length,
                    "total": newProducts1.length + newProducts2.length
                },
                oldProductsPair,
                newProductsPair
            },
            "riskLimits": {
                "old": oldRiskLimits.length,
                "new": newRiskLimits.length
            },
            "riskLimitsV2": {
                "old": oldRiskLimitsV2.length,
                "new": newRiskLimitsV2.length
            },
            "leverages": {
                "old": oldLeverages.length,
                "new": newLeverages.length
            }
        }
        //
        //     {
